...
import BookModel from "../app/modules/Books/book.model";
...
async function seedBooks() {
  try {
    ...
    const user = await UserRegModel.findOne({ isDeleted: false });
    ...
    const booksWithUser = dummyBooks.map((book) => ({
      ...book,
      user: user._id,
      isDeleted: false,
      isPublished: true,
    ...
    // Insert books
    const result = await BookModel.insertMany(booksWithUser);
